...
u16 Hw_IMU_Update( HW_IMU_DATA_OBJ *ptr_data, u32 dt )
{
	...
	if( Err == 0 )
	{
		Hw_IMU_SaveData.X_Angle = ypr[1];
		Hw_IMU_SaveData.Y_Angle = ypr[2];
	...
	if( Err == 0 )
	{
		...
		*/
		...
		/*	
		Hw_FreeIMU_GetUpdateQ( Q, raw_value, dt_sec ); 

		Hw_FreeIMU_GetYawPitchRoll( Q, ypr );
		*/

		Hw_IMU_FreeIMU_getYawPitchRoll( ypr );

		ptr_data->X_Angle = ypr[1];
		ptr_data->Y_Angle = ypr[2];
		ptr_data->Z_Angle = ypr[0];
